...
function calculateScore(scoreidx, dices) {
    function c(n) { return dices.filter(x => x===n).length }
    ...
    switch (scoreRules[scoreidx]) {
        ...
        case '3개':
            if (c(1)>=3 || c(2)>=3 || c(3)>=3 || c(4)>=3 || c(5)>=3 || c(6)>=3)
                return dice.reduce((a,b)=>a+b)
            return 0
        case '4개':
            if (c(1)>=4 || c(2)>=4 || c(3)>=4 || c(4)>=4 || c(5)>=4 || c(6)>=4)
                return dice.reduce((a,b)=>a+b)
            return 0
        case '풀하우스': {
            const tmp = [...dice].sort()
            if (tmp[0]===tmp[1] && tmp[3]===tmp[4] && (tmp[2]===tmp[0] || tmp[2]===tmp[4]))
                return 25
            return 0
        }
        case '스트레이트': {
